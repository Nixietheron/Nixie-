...
Every alert is sent as a Telegram photo card, with the Nixie after-hours visual, the buy amount and buyer wallet in the caption, plus Noxa and Dexscreener buttons. `public/nix-buy-alert.png` is used by default; set `NIX_BUY_ALERT_IMAGE_URL` only when you host a different HTTPS image.
